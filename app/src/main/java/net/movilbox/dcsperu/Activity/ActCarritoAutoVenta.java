...
import net.movilbox.dcsperu.Adapter.AdapterCarritoAutoventa;
...
/**
 * Created by jhonjimenez on 31/10/16.
 */

public class ActCarritoAutoVenta extends AppCompatActivity {
    ...
    private AdapterCarritoAutoventa adapterCarrito;
    ...
    private void borrarCarrito() {
        ...
        builder.setPositiveButton("Eliminar", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int id) {
                //Toast.makeText(ActCarritoAutoVenta.this,""+id_punto,Toast.LENGTH_LONG).show();
                boolean result =  mydb.deleteAllCarritoAutoventa(id_punto);
                if(result){
                    Toast.makeText(ActCarritoAutoVenta.this,"El carrito se a eliminado",Toast.LENGTH_LONG).show();
                    simsList.clear();
                    adapterCarrito.notifyDataSetChanged();
                    refresh = 1;
                    calculaTotal();
                }else{
                    Toast.makeText(ActCarritoAutoVenta.this,"No se pudo eliminar el carrito",Toast.LENGTH_LONG).show();
                }
            }
    ...
    private void llenarDataCarrito(int id_punto) {

        simsList = mydb.getCarritoAutoventa(id_punto);
        calculaTotal();
        adapterCarrito = new AdapterCarritoAutoventa(this, simsList);
    ...
    private void deletePrduct(final int position) {
        ...
        builder.setPositiveButton("Eliminar", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int id) {
                ...
                if (!mydb.deleteCarritoProductoAutoventa(simsList.get(position).getId_auto_carrito(), simsList.get(position).getId_punto())) {
                    Toast.makeText(getApplicationContext(), "Problemas al eliminar el producto", Toast.LENGTH_SHORT).show();
                } else {
                    simsList.clear();
                    llenarDataCarrito(id_punto);
                    adapterCarrito = new AdapterCarritoAutoventa(ActCarritoAutoVenta.this, simsList);
                    mListView.setAdapter(adapterCarrito);
                    calculaTotal();
                    refresh = 1;
    ...
    private void savePedido() {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle("¿ Esta seguro que desea guardar la venta ?");
        builder.setCancelable(false);
            ...
            public void onClick(DialogInterface dialog, int id) {
                if(simsList.size() > 0){
                    if (connectionDetector.isConnected()) {
                        ...
                        saveLocalDada();
                        Toast.makeText(ActCarritoAutoVenta.this,"no internet",Toast.LENGTH_LONG).show();
                    }
                }else{
                    Toast.makeText(ActCarritoAutoVenta.this,"No hay productos en el carrito",Toast.LENGTH_LONG).show();
                }
            }
        }).setNegativeButton("Cancelar", new DialogInterface.OnClickListener() {
            @Override
    ...
    private void saveLocalDada() {
        ...
        if (mydb.insertAutoventaOffLine(simsList, mydb.getUserLogin().getId(), mydb.getUserLogin().getId_distri(), mydb.getUserLogin().getBd(), id_punto, gpsServices.getLatitude(), gpsServices.getLongitude(), id_comprobante)) {
            if (mydb.deleteAllCarritoAutoventa(id_punto)) {
                ...
                new Response.ErrorListener() {
                    ...
                    public void onErrorResponse(VolleyError error) {
                        ...
                        if (error instanceof TimeoutError || error instanceof NoConnectionError) {
                            Toast.makeText(ActCarritoAutoVenta.this, "Error de tiempo de espera", Toast.LENGTH_LONG).show();
                        } else if (error instanceof AuthFailureError) {
                            Toast.makeText(ActCarritoAutoVenta.this, "Error Servidor", Toast.LENGTH_LONG).show();
                        } else if (error instanceof ServerError) {
                            Toast.makeText(ActCarritoAutoVenta.this, "Server Error", Toast.LENGTH_LONG).show();
                        } else if (error instanceof NetworkError) {
                            Toast.makeText(ActCarritoAutoVenta.this, "Error de red", Toast.LENGTH_LONG).show();
                        } else if (error instanceof ParseError) {
                            Toast.makeText(ActCarritoAutoVenta.this, "Error al serializar los datos", Toast.LENGTH_LONG).show();
                        }
